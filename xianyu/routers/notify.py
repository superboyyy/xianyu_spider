...
from xianyu.notify.service import CHANNEL_KINDS, dispatch_notify, fingerprint
...
async def list_kinds():
    return {
        "items": [
            ...
            {"id": "ntfy", "label": "ntfy", "placeholder": "https://ntfy.sh/topic 或 token topic"},
            {"id": "telegram", "label": "Telegram", "placeholder": "bot_token|chat_id"},
            {"id": "serverchan", "label": "Server酱", "placeholder": "SCT 的 SendKey"},
            {"id": "wecom", "label": "企业微信机器人", "placeholder": "key 或完整 webhook URL"},
        ]
    }
...
async def create_channel(body: NotifyChannelBody):
    ...
    if kind not in CHANNEL_KINDS:
        raise HTTPException(status_code=400, detail=f"kind 仅支持 {' / '.join(CHANNEL_KINDS)}")
