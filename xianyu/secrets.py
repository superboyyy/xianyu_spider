...
def normalize_autoreply_mode(mode: Any) -> str:
    text = str(mode or "off").strip().lower()
    if text == "send":
        return "draft"
    if text in AUTOREPLY_MODES:
        return text
    return "off"
...
def save_keys(data: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        k: ("***" if k.endswith("_key") or k.endswith("token") or "key" in k.lower() else v)
...
def load_settings() -> dict[str, Any]:
    ...
    merged.update(data)
    merged["autoreply_mode"] = normalize_autoreply_mode(merged.get("autoreply_mode"))
    merged["ai_provider"] = str(merged.get("ai_provider") or "auto").strip().lower() or "auto"
    return merged
...
def save_settings(patch: dict[str, Any]) -> dict[str, Any]:
    ...
    for key, value in patch.items():
        if key == "autoreply_mode":
            current[key] = normalize_autoreply_mode(value)
            continue
...
def masked_settings() -> dict[str, Any]:
    ...
    return {
        **settings,
        ...
        "autoreply_modes": sorted(AUTOREPLY_MODES),
    }
