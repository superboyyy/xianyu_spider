<template>
  <div class="split">
    <form class="panel stack" @submit.prevent="create">
      ...
      <label>间隔（分钟）<input v-model.number="form.interval_minutes" type="number" min="5" /></label>
      ...
      <p class="note">调度在引擎进程内运行；自动跑最短 5 分钟一次。关掉服务任务会停。</p>
    </form>
...
onMounted(async () => {
  if (route.query.keyword) form.keyword = String(route.query.keyword);
  if (route.query.target) form.target_price = String(route.query.target);
  try {
    await load();
  } catch (err) {
    error.value = err.message;
